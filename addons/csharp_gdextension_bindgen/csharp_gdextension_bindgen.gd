...
static func generate_csharp_script(
	...
	var csharp_parent_class = _get_csharp_class_name(parent_class)
	var engine_class = _get_csharp_class_name(_first_non_extension_parent(cls_name))
	...
	if not parent_class_is_extension:
		regions.append((
			"// Engine object used for calling engine methods\n"
			+ "protected %s _object;\n\n" % csharp_parent_class
			+ "public %s NativeObject => _object;" % engine_class
		))
	...
	""".dedent().format({
		...
		cls_name = cls_name,
		inheritance = " : " + csharp_parent_class if parent_class_is_extension else "",
		regions = "\n\n".join(regions).indent("\t"),
	}).strip_edges()
...
static func _generate_enum(cls_name: StringName, enum_name: StringName) -> String:
	...
	for constant_name in ClassDB.class_get_enum_constants(cls_name, enum_name, true):
		constants.append("{csharp_constant_name} = {constant_value}L,".format({
			csharp_constant_name = _make_valid_csharp_identifier(
				constant_name.substr(common_prefix.length()).to_pascal_case()
			),
...
static func _generate_method(cls_name: StringName, method: Dictionary) -> String:
	...
	for default_value in default_args:
		...
		elif typeof(default_value) == TYPE_ARRAY:
	...
	if _is_extension_class(cls_name):
		...
		if is_static:
			implementation.append("{maybe_return}{engine_class}.{csharp_method_name}({arg_names});".format({
				arg_names = ", ".join(arg_names),
				engine_class = _get_csharp_class_name(_first_non_extension_parent(cls_name)),
				csharp_method_name = csharp_method_name,
...
static func _generate_signal(cls_name: StringName, sig: Dictionary):
	...
	return """
		public event {delegate_type} {csharp_signal_name}
		{
			add
			{
				_object.Connect(SignalName.{csharp_signal_name}, Callable.From(value));
			}
			remove
			{
				_object.Disconnect(SignalName.{csharp_signal_name}, Callable.From(value));
			}
		}
	""".dedent().format({
	...
	match property["type"]:
		...
		TYPE_INT:
			if _property_is_enum(property):
				...
				if split.size() == 1:
					...
					return (
						_get_csharp_class_name(split[0]) + "." + split[1]
						+ ("Enum" if _needs_enum_suffix(split[0], split[1]) else "")
					)
			return "int"
		TYPE_FLOAT:
		...
		TYPE_ARRAY:
			if property["hint"] & PROPERTY_HINT_ARRAY_TYPE:
				var native_element_type: String = property["hint_string"]
				var element_type = _get_mapped_variant_type(native_element_type)
				# Generated extension wrappers do not derive from GodotObject, so they
				# cannot satisfy Godot.Collections.Array<T>'s generic constraint.
				# Keep such arrays untyped; each element can still be explicitly cast
				# to its generated wrapper through Variant.
				if ClassDB.class_exists(native_element_type) and _is_extension_class(native_element_type):
					return "Godot.Collections.Array"
				return "Godot.Collections.Array<%s>" % element_type
			else:
				return "Godot.Collections.Array"
		TYPE_DICTIONARY:
...
static func _get_mapped_variant_type(variant_type_name: String) -> String:
	...
	if _type_map.has(variant_type_name):
		return _type_map[variant_type_name]
	if ClassDB.class_exists(variant_type_name):
		return _get_csharp_class_name(variant_type_name)
	return "Godot." + _pascal_to_pascal_case(variant_type_name)
...
static func _get_csharp_class_name(cls_name: StringName) -> String:
	if ClassDB.class_exists(cls_name) and _is_extension_class(cls_name):
		return cls_name
	return _pascal_to_pascal_case(cls_name)
...
static func _generate_strings_class(cls_name: StringName, string_name_type: StringNameType, string_names: PackedStringArray) -> String:
	...
	""".dedent().format({
		...
		parent_class = csharp_parent_class,
...
static func _make_valid_csharp_identifier(identifier: String) -> String:
	# Godot enum values may legally begin with a digit (for example SCALE_1X),
	# while C# identifiers may not. Preserve the reflected name as closely as
	# possible and add a descriptive prefix only when it is required.
	if not identifier.is_empty() and _is_digit(identifier[0]):
